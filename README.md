...
Comparisons include a windowed P95 time-series overlay, an `overall_p95` bar
chart based on Locust's cumulative whole-run P95, and a `mean_window_p95` bar
chart showing the mean P95 across the configured `p95_window_s` windows.
...
window P95, mean throughput, mean pod creation-to-scheduled time, and mean
replica count. The
